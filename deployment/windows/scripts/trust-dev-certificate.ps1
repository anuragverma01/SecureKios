...
if ($certificate.EnhancedKeyUsageList.ObjectId.Value -notcontains '1.3.6.1.5.5.7.3.3') { throw 'Refusing to trust a certificate that is not valid for code signing.' }

Import-Certificate -FilePath $CertificatePath -CertStoreLocation Cert:\LocalMachine\TrustedPeople | Out-Null
$installed = Get-ChildItem Cert:\LocalMachine\TrustedPeople\$($certificate.Thumbprint) -ErrorAction SilentlyContinue
